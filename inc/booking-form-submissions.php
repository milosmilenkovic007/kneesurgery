<?php

if (!defined('ABSPATH')) {
    exit;
}

if (!defined('HJ_BFS_POST_TYPE')) {
    define('HJ_BFS_POST_TYPE', 'hj_booking_form');
}

if (!function_exists('hj_bfs_register_post_type')) {
    function hj_bfs_register_post_type()
    {
        register_post_type(HJ_BFS_POST_TYPE, [
            'labels' => [
                'name' => __('Submissions', 'kneesurgery'),
                'singular_name' => __('Submission', 'kneesurgery'),
                'menu_name' => __('Forms', 'kneesurgery'),
                'all_items' => __('Submissions', 'kneesurgery'),
                'edit_item' => __('View Submission', 'kneesurgery'),
                'view_item' => __('View Submission', 'kneesurgery'),
                'search_items' => __('Search Submissions', 'kneesurgery'),
                'not_found' => __('No submissions found.', 'kneesurgery'),
                'not_found_in_trash' => __('No submissions found in Trash.', 'kneesurgery'),
            ],
            'public' => false,
            'publicly_queryable' => false,
            'show_ui' => true,
            'show_in_menu' => true,
            'show_in_admin_bar' => false,
            'show_in_nav_menus' => false,
            'exclude_from_search' => true,
            'has_archive' => false,
            'rewrite' => false,
            'query_var' => false,
            'menu_position' => 26,
            'menu_icon' => 'dashicons-email-alt2',
            'supports' => ['title'],
            'map_meta_cap' => true,
            'capabilities' => [
                'create_posts' => 'do_not_allow',
            ],
        ]);
    }
}
add_action('init', 'hj_bfs_register_post_type');

    if (!function_exists('hj_bfs_redirect_legacy_admin_url')) {
        function hj_bfs_redirect_legacy_admin_url()
        {
            if (!is_admin()) {
                return;
            }

            $post_type = isset($_GET['post_type']) ? sanitize_key(wp_unslash($_GET['post_type'])) : '';
            if ($post_type !== 'hj_booking_submission') {
                return;
            }

            $redirect_url = add_query_arg(
                [
                    'post_type' => HJ_BFS_POST_TYPE,
                ],
                admin_url('edit.php')
            );

            wp_safe_redirect($redirect_url);
            exit;
    }
}
    add_action('admin_init', 'hj_bfs_redirect_legacy_admin_url');

if (!function_exists('hj_bfs_create_submission')) {
    function hj_bfs_create_submission(array $data)
    {
        $full_name = trim((string) ($data['full_name'] ?? ''));
        $email = trim((string) ($data['email'] ?? ''));
        $phone = trim((string) ($data['phone'] ?? ''));
        $preferred_contact_method = trim((string) ($data['preferred_contact_method'] ?? ''));
        $country_code = trim((string) ($data['country_code'] ?? ''));
        $treatment_id = absint($data['treatment_id'] ?? 0);
        $treatment_title = trim((string) ($data['treatment_title'] ?? ''));
        $treatment_price = trim((string) ($data['treatment_price'] ?? ''));
        $source_url = esc_url_raw((string) ($data['source_url'] ?? ''));
        $source = trim((string) ($data['source'] ?? ''));
        $email_status = sanitize_key((string) ($data['email_status'] ?? ''));
        $zoho_status = sanitize_key((string) ($data['zoho_status'] ?? ''));
        $zoho_code = absint($data['zoho_code'] ?? 0);
        $zoho_response = trim((string) ($data['zoho_response'] ?? ''));

        $submission_title = sprintf(
            __('%1$s - %2$s - %3$s', 'kneesurgery'),
            $full_name !== '' ? $full_name : __('Booking enquiry', 'kneesurgery'),
            $treatment_title !== '' ? $treatment_title : __('Treatment package', 'kneesurgery'),
            wp_date('Y-m-d H:i')
        );

        $submission_id = wp_insert_post([
            'post_type' => HJ_BFS_POST_TYPE,
            'post_status' => 'publish',
            'post_title' => $submission_title,
        ], true);

        if (is_wp_error($submission_id)) {
            return $submission_id;
        }

        update_post_meta($submission_id, '_hj_bfs_full_name', $full_name);
        update_post_meta($submission_id, '_hj_bfs_email', $email);
        update_post_meta($submission_id, '_hj_bfs_phone', $phone);
        update_post_meta($submission_id, '_hj_bfs_preferred_contact_method', $preferred_contact_method);
        update_post_meta($submission_id, '_hj_bfs_country_code', $country_code);
        update_post_meta($submission_id, '_hj_bfs_treatment_id', $treatment_id);
        update_post_meta($submission_id, '_hj_bfs_treatment_title', $treatment_title);
        update_post_meta($submission_id, '_hj_bfs_treatment_price', $treatment_price);
        update_post_meta($submission_id, '_hj_bfs_source', $source);
        update_post_meta($submission_id, '_hj_bfs_source_url', $source_url);
        update_post_meta($submission_id, '_hj_bfs_email_status', $email_status);
        update_post_meta($submission_id, '_hj_bfs_zoho_status', $zoho_status);
        update_post_meta($submission_id, '_hj_bfs_zoho_code', $zoho_code);

        if ($zoho_response !== '') {
            update_post_meta($submission_id, '_hj_bfs_zoho_response', $zoho_response);
        }

        return $submission_id;
    }
}

if (!function_exists('hj_bfs_get_submission_meta')) {
    function hj_bfs_get_submission_meta($post_id, $key)
    {
        return (string) get_post_meta($post_id, $key, true);
    }
}

if (!function_exists('hj_bfs_can_retry_zoho_submission')) {
    function hj_bfs_can_retry_zoho_submission($post_id)
    {
        return get_post_type($post_id) === HJ_BFS_POST_TYPE
            && hj_bfs_get_submission_meta($post_id, '_hj_bfs_zoho_status') === 'failed';
    }
}

if (!function_exists('hj_bfs_get_submission_lead')) {
    function hj_bfs_get_submission_lead($post_id)
    {
        return [
            'full_name' => hj_bfs_get_submission_meta($post_id, '_hj_bfs_full_name'),
            'email' => hj_bfs_get_submission_meta($post_id, '_hj_bfs_email'),
            'phone' => hj_bfs_get_submission_meta($post_id, '_hj_bfs_phone'),
            'preferred_contact_method' => hj_bfs_get_submission_meta($post_id, '_hj_bfs_preferred_contact_method'),
            'country_code' => hj_bfs_get_submission_meta($post_id, '_hj_bfs_country_code'),
            'service' => hj_bfs_get_submission_meta($post_id, '_hj_bfs_treatment_title'),
            'price' => hj_bfs_get_submission_meta($post_id, '_hj_bfs_treatment_price'),
            'source' => hj_bfs_get_submission_meta($post_id, '_hj_bfs_source'),
            'source_url' => hj_bfs_get_submission_meta($post_id, '_hj_bfs_source_url'),
        ];
    }
}

if (!function_exists('hj_bfs_get_retry_redirect_url')) {
    function hj_bfs_get_retry_redirect_url($fallback = '')
    {
        $fallback = $fallback !== '' ? $fallback : admin_url('edit.php?post_type=' . HJ_BFS_POST_TYPE);
        $redirect_to = isset($_REQUEST['_wp_http_referer']) ? wp_unslash($_REQUEST['_wp_http_referer']) : '';
        if ($redirect_to === '') {
            $redirect_to = wp_get_referer();
        }

        return wp_validate_redirect($redirect_to, $fallback);
    }
}

if (!function_exists('hj_bfs_get_retry_zoho_url')) {
    function hj_bfs_get_retry_zoho_url($post_id)
    {
        return wp_nonce_url(
            add_query_arg(
                [
                    'action' => 'hj_bfs_retry_zoho_submission',
                    'submission_id' => absint($post_id),
                ],
                admin_url('admin-post.php')
            ),
            'hj_bfs_retry_zoho_submission_' . absint($post_id)
        );
    }
}

if (!function_exists('hj_bfs_format_delivery_status')) {
    function hj_bfs_format_delivery_status($status, $channel = 'zoho')
    {
        $status = sanitize_key((string) $status);
        if ($status === '') {
            return '';
        }

        if ($channel === 'email') {
            $labels = [
                'sent' => 'Sent',
                'failed' => 'Failed',
                'skipped' => 'Skipped',
                'disabled' => 'Disabled',
            ];

            if (isset($labels[$status])) {
                return $labels[$status];
            }
        }

        if ($channel === 'zoho') {
            $labels = [
                'sent' => 'Sent',
                'duplicate_handled' => 'Duplicate handled (sent)',
                'queued' => 'Queued',
                'failed' => 'Failed',
                'disabled' => 'Disabled',
                'unavailable' => 'Unavailable',
                'misconfigured' => 'Misconfigured',
            ];

            if (isset($labels[$status])) {
                return $labels[$status];
            }
        }

        return ucfirst(str_replace('_', ' ', $status));
    }
}

add_filter('manage_' . HJ_BFS_POST_TYPE . '_posts_columns', function ($columns) {
    return [
        'cb' => $columns['cb'] ?? '<input type="checkbox" />',
        'title' => __('Submission', 'kneesurgery'),
        'hj_bfs_treatment' => __('Treatment', 'kneesurgery'),
        'hj_bfs_email' => __('Email', 'kneesurgery'),
        'hj_bfs_phone' => __('Mobile', 'kneesurgery'),
        'hj_bfs_delivery' => __('Delivery', 'kneesurgery'),
        'date' => __('Submitted', 'kneesurgery'),
    ];
});

add_action('manage_' . HJ_BFS_POST_TYPE . '_posts_custom_column', function ($column, $post_id) {
    switch ($column) {
        case 'hj_bfs_treatment':
            echo esc_html(hj_bfs_get_submission_meta($post_id, '_hj_bfs_treatment_title'));
            break;
        case 'hj_bfs_email':
            $email = hj_bfs_get_submission_meta($post_id, '_hj_bfs_email');
            if ($email !== '') {
                echo '<a href="mailto:' . esc_attr($email) . '">' . esc_html($email) . '</a>';
            }
            break;
        case 'hj_bfs_phone':
            echo esc_html(hj_bfs_get_submission_meta($post_id, '_hj_bfs_phone'));
            break;
        case 'hj_bfs_delivery':
            $email_status = hj_bfs_format_delivery_status(hj_bfs_get_submission_meta($post_id, '_hj_bfs_email_status'), 'email');
            $zoho_status = hj_bfs_format_delivery_status(hj_bfs_get_submission_meta($post_id, '_hj_bfs_zoho_status'), 'zoho');
            $parts = [];
            if ($email_status !== '') {
                $parts[] = 'Email: ' . $email_status;
            }
            if ($zoho_status !== '') {
                $parts[] = 'Zoho: ' . $zoho_status;
            }
            if (!empty($parts)) {
                echo esc_html(implode(' | ', $parts));
            } else {
                echo '&mdash;';
            }

            if (hj_bfs_can_retry_zoho_submission($post_id) && current_user_can('edit_post', $post_id)) {
                echo '<br><a class="button button-small" href="' . esc_url(hj_bfs_get_retry_zoho_url($post_id)) . '">' . esc_html__('Retry Zoho', 'kneesurgery') . '</a>';
            }
            break;
    }
}, 10, 2);

add_filter('post_row_actions', function ($actions, $post) {
    if (!$post instanceof WP_Post || $post->post_type !== HJ_BFS_POST_TYPE) {
        return $actions;
    }

    unset($actions['inline hide-if-no-js']);
    unset($actions['view']);

    if (hj_bfs_can_retry_zoho_submission($post->ID) && current_user_can('edit_post', $post->ID)) {
        $actions['hj_bfs_retry_zoho'] = '<a href="' . esc_url(hj_bfs_get_retry_zoho_url($post->ID)) . '">' . esc_html__('Retry Zoho', 'kneesurgery') . '</a>';
    }

    return $actions;
}, 10, 2);

add_action('add_meta_boxes_' . HJ_BFS_POST_TYPE, function () {
    add_meta_box(
        'hj-bfs-details',
        __('Submission Details', 'kneesurgery'),
        function ($post) {
            $fields = [
                __('Full name', 'kneesurgery') => hj_bfs_get_submission_meta($post->ID, '_hj_bfs_full_name'),
                __('Email', 'kneesurgery') => hj_bfs_get_submission_meta($post->ID, '_hj_bfs_email'),
                __('Mobile', 'kneesurgery') => hj_bfs_get_submission_meta($post->ID, '_hj_bfs_phone'),
                __('Preferred contact method', 'kneesurgery') => hj_bfs_get_submission_meta($post->ID, '_hj_bfs_preferred_contact_method'),
                __('Country', 'kneesurgery') => strtoupper(hj_bfs_get_submission_meta($post->ID, '_hj_bfs_country_code')),
                __('Treatment', 'kneesurgery') => hj_bfs_get_submission_meta($post->ID, '_hj_bfs_treatment_title'),
                __('Price', 'kneesurgery') => hj_bfs_get_submission_meta($post->ID, '_hj_bfs_treatment_price'),
                __('Source', 'kneesurgery') => hj_bfs_get_submission_meta($post->ID, '_hj_bfs_source'),
                __('Source URL', 'kneesurgery') => hj_bfs_get_submission_meta($post->ID, '_hj_bfs_source_url'),
                __('Email Delivery', 'kneesurgery') => hj_bfs_format_delivery_status(hj_bfs_get_submission_meta($post->ID, '_hj_bfs_email_status'), 'email'),
                __('Zoho Status', 'kneesurgery') => hj_bfs_get_submission_meta($post->ID, '_hj_bfs_zoho_status'),
                __('Zoho HTTP Code', 'kneesurgery') => hj_bfs_get_submission_meta($post->ID, '_hj_bfs_zoho_code'),
                __('Submitted', 'kneesurgery') => get_the_date('Y-m-d H:i:s', $post),
            ];

            if (hj_bfs_can_retry_zoho_submission($post->ID) && current_user_can('edit_post', $post->ID)) {
                echo '<p><a class="button button-secondary" href="' . esc_url(hj_bfs_get_retry_zoho_url($post->ID)) . '">' . esc_html__('Retry Zoho', 'kneesurgery') . '</a></p>';
            }

            echo '<table class="widefat striped" style="border:0">';
            foreach ($fields as $label => $value) {
                echo '<tr>';
                echo '<td style="width:180px"><strong>' . esc_html($label) . '</strong></td>';
                echo '<td>';
                if (filter_var($value, FILTER_VALIDATE_URL)) {
                    echo '<a href="' . esc_url($value) . '" target="_blank" rel="noopener">' . esc_html($value) . '</a>';
                } elseif (is_email($value)) {
                    echo '<a href="mailto:' . esc_attr($value) . '">' . esc_html($value) . '</a>';
                } else {
                    echo esc_html($value);
                }
                echo '</td>';
                echo '</tr>';
            }
            echo '</table>';
        },
        HJ_BFS_POST_TYPE,
        'normal',
        'high'
    );
});

add_action('admin_menu', function () {
    remove_submenu_page('edit.php?post_type=' . HJ_BFS_POST_TYPE, 'post-new.php?post_type=' . HJ_BFS_POST_TYPE);
}, 99);

add_action('admin_post_hj_bfs_retry_zoho_submission', function () {
    $submission_id = isset($_GET['submission_id']) ? absint($_GET['submission_id']) : 0;
    $fallback_url = admin_url('edit.php?post_type=' . HJ_BFS_POST_TYPE);

    if ($submission_id <= 0 || get_post_type($submission_id) !== HJ_BFS_POST_TYPE) {
        wp_safe_redirect(add_query_arg('hj_bfs_zoho_retry', 'invalid', $fallback_url));
        exit;
    }

    if (!current_user_can('edit_post', $submission_id)) {
        wp_die(esc_html__('You do not have permission to retry this submission.', 'kneesurgery'), 403);
    }

    check_admin_referer('hj_bfs_retry_zoho_submission_' . $submission_id);

    $redirect_url = hj_bfs_get_retry_redirect_url(get_edit_post_link($submission_id, ''));

    if (!hj_bfs_can_retry_zoho_submission($submission_id)) {
        wp_safe_redirect(add_query_arg('hj_bfs_zoho_retry', 'not_failed', $redirect_url));
        exit;
    }

    if (!function_exists('hj_send_zoho_crm_lead')) {
        wp_safe_redirect(add_query_arg('hj_bfs_zoho_retry', 'unavailable', $redirect_url));
        exit;
    }

    $result = hj_send_zoho_crm_lead(hj_bfs_get_submission_lead($submission_id), false);

    $status = sanitize_key((string) ($result['status'] ?? 'failed'));
    $code = (int) ($result['code'] ?? 0);
    $body = (string) ($result['body'] ?? '');

    update_post_meta($submission_id, '_hj_bfs_zoho_status', $status);
    update_post_meta($submission_id, '_hj_bfs_zoho_code', $code);

    if ($body !== '') {
        update_post_meta($submission_id, '_hj_bfs_zoho_response', $body);
    } else {
        delete_post_meta($submission_id, '_hj_bfs_zoho_response');
    }

    wp_safe_redirect(add_query_arg([
        'hj_bfs_zoho_retry' => $status === 'sent' ? 'sent' : 'failed',
        'hj_bfs_zoho_retry_code' => $code,
    ], $redirect_url));
    exit;
});

add_action('admin_notices', function () {
    if (!is_admin()) {
        return;
    }

    $screen = get_current_screen();
    if (!$screen || $screen->post_type !== HJ_BFS_POST_TYPE) {
        return;
    }

    $retry_status = isset($_GET['hj_bfs_zoho_retry']) ? sanitize_key(wp_unslash($_GET['hj_bfs_zoho_retry'])) : '';
    if ($retry_status === '') {
        return;
    }

    $http_code = isset($_GET['hj_bfs_zoho_retry_code']) ? absint($_GET['hj_bfs_zoho_retry_code']) : 0;
    $notice_type = $retry_status === 'sent' ? 'updated' : 'error';
    $messages = [
        'sent' => __('Zoho lead retried successfully.', 'kneesurgery'),
        'failed' => __('Zoho retry failed.', 'kneesurgery'),
        'invalid' => __('Invalid submission selected for Zoho retry.', 'kneesurgery'),
        'not_failed' => __('Retry is available only for submissions with failed Zoho status.', 'kneesurgery'),
        'unavailable' => __('Zoho retry is currently unavailable.', 'kneesurgery'),
    ];

    $message = $messages[$retry_status] ?? __('Zoho retry status is unknown.', 'kneesurgery');
    if ($http_code > 0) {
        $message .= ' ' . sprintf(__('HTTP code: %d', 'kneesurgery'), $http_code);
    }

    echo '<div class="notice ' . esc_attr($notice_type) . ' is-dismissible"><p>' . esc_html($message) . '</p></div>';
});

add_action('admin_head', function () {
    $screen = get_current_screen();

    if (!$screen || $screen->post_type !== HJ_BFS_POST_TYPE) {
        return;
    }

    echo '<style>.post-type-' . esc_attr(HJ_BFS_POST_TYPE) . ' .page-title-action,.post-type-' . esc_attr(HJ_BFS_POST_TYPE) . ' #minor-publishing-actions,.post-type-' . esc_attr(HJ_BFS_POST_TYPE) . ' #misc-publishing-actions .misc-pub-post-status,.post-type-' . esc_attr(HJ_BFS_POST_TYPE) . ' #misc-publishing-actions .misc-pub-visibility{display:none!important;}</style>';
});