...
if (!function_exists('hj_spc_extract_numeric_price')) {
    function hj_spc_extract_numeric_price($value)
    {
        $value = trim((string) $value);
        if ($value === '') {
            return '';
...
if (!function_exists('hj_spc_prepare_source_url_for_crm')) {
    function hj_spc_prepare_source_url_for_crm($url)
    {
        $url = trim((string) $url);
        if ($url === '') {
            return '';
...
if (!function_exists('hj_spc_split_full_name')) {
    function hj_spc_split_full_name($full_name)
    {
...
if (!function_exists('hj_spc_send_zoho_plugin_lead')) {
    function hj_spc_send_zoho_plugin_lead(array $lead)
    {
        if (!function_exists('zoho_crm_send_lead_data')) {
            return ['status' => 'unavailable', 'code' => 0, 'body' => 'Zoho plugin sender unavailable.'];
        }

        $name = hj_spc_split_full_name((string) ($lead['full_name'] ?? ''));
        $service = trim((string) ($lead['service'] ?? ''));
        $price = trim((string) ($lead['price'] ?? ''));
        $source_url = hj_spc_prepare_source_url_for_crm((string) ($lead['source_url'] ?? ''));
        $message_parts = [];
        if ($service !== '') {
            $message_parts[] = 'Treatment: ' . $service;
        }
        if ($price !== '') {
            $message_parts[] = 'Price: ' . $price;
        }

        $lead_data = [
            'Full_Name' => (string) $name['full_name'],
            'Last_Name' => (string) ($name['last_name'] !== '' ? $name['last_name'] : 'Website Lead'),
            'Contact_Through' => (string) ($lead['preferred_contact_method'] ?? ''),
            'Email' => (string) ($lead['email'] ?? ''),
            'Phone' => (string) ($lead['phone'] ?? ''),
            'Mobile' => (string) ($lead['phone'] ?? ''),
            'Multi_Line_7' => implode(' | ', $message_parts),
            'Lead_Source' => 'Kneesurgery Website',
            'Lead_Status' => 'New',
            'Lead_Source_URL' => $source_url,
        ];

        return zoho_crm_send_lead_data(
            $lead_data,
            (string) ($name['first_name'] !== '' ? $name['first_name'] : $name['full_name']),
            'Custom form lead error: '
        );
    }
}
...
if (!function_exists('hj_spc_handle_lead_submission')) {
    function hj_spc_handle_lead_submission()
    {
        ...
        $treatment_price = isset($_POST['treatment_price']) ? sanitize_text_field(wp_unslash($_POST['treatment_price'])) : '';
        $source = 'Kneesurgery Website';

        if ($treatment_id > 0 && function_exists('get_field')) {
            $stored_price = get_field('package_includes_price', $treatment_id);
            if (is_scalar($stored_price)) {
                $stored_price = trim((string) $stored_price);
                if ($stored_price !== '') {
                    $treatment_price = $stored_price;
                }
            }
        }

        $treatment_price_numeric = hj_spc_extract_numeric_price($treatment_price);
        if ($treatment_price_numeric !== '') {
            $treatment_price = $treatment_price_numeric;
        }
        ...
        $zoho_result = hj_spc_send_zoho_plugin_lead([
                'full_name' => $full_name,
                'email' => $email,
                'phone' => $formatted_phone,
                'preferred_contact_method' => $preferred_contact_method,
                'country_code' => $country_code,
                'service' => $treatment_title,
        ...
        $submission_id = function_exists('hj_bfs_create_submission')
            ? hj_bfs_create_submission([
                ...
                'zoho_status' => sanitize_key((string) ($zoho_result['status'] ?? '')),
                'zoho_code' => (int) ($zoho_result['code'] ?? 0),
                'zoho_response' => (string) ($zoho_result['body'] ?? ''),
            ])
            : 0;
        ...
        if ($submission_id > 0) {
            update_post_meta($submission_id, '_hj_bfs_email_status', $sent ? 'sent' : 'failed');
        }
